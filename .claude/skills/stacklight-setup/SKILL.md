...
- **Schema**: `.claude/skills/stacklight-setup/providers.schema.json` — the single source of truth for which fields each provider needs. Never hard-code field names; always read this file.
- **Validator**: `.claude/skills/stacklight-setup/scripts/validate.sh <provider> <json-config>` — returns exit 0 on success, non-zero with a short error on failure.
...
Read `.claude/skills/stacklight-setup/providers.schema.json`. The `providers` object is authoritative — every field, secret flag, required flag, and test endpoint comes from here.
...
```bash
.claude/skills/stacklight-setup/scripts/validate.sh <provider> <path-to-partial-json>
```

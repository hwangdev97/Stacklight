...
struct MenuBarContentView: View {
    ...
    /// Measured height of the full menu content (provider sections + footer).
    /// Drives the scroll frame so a short menu sizes the window to its content
    /// (no empty space) while a long one is capped and scrolls instead of
    /// growing off-screen.
    @State private var contentHeight: CGFloat = 0

    /// Upper bound on the panel height. The entire menu — provider sections
    /// and footer alike — scrolls as one region, so a long deployment list
    /// scrolls past Refresh/Settings/Quit instead of pushing the panel
    /// off-screen.

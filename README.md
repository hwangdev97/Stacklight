...
ShapeBar lives in your menu bar and gives you a quick glance at the status of your deployments and pull requests. No need to open dashboards or switch between tabs — just click the triangle icon.
...
```bash
# Clone the repo
git clone https://github.com/hwangdev97/ShapeBar.git
cd ShapeBar
...
> **Note:** ShapeBar is a menu bar app (`LSUIElement`). It has no Dock icon — look for the **triangle icon** (▲) in the menu bar after launching.
...
ShapeBar uses a plugin architecture — each service implements the `DeploymentProvider` protocol and is auto-registered in `ServiceRegistry`. Adding a new service is as simple as creating a new provider file.
...
ShapeBar 是一个 macOS 菜单栏应用，用于集中监控多个平台的部署状态、CI/CD 流水线和 GitHub Pull Requests。无需打开各种仪表盘，点一下菜单栏图标即可查看。
...
> **注意：** ShapeBar 是菜单栏应用（`LSUIElement`），没有 Dock 图标。启动后请在菜单栏寻找 **三角形图标**（▲）。
...
ShapeBar 采用插件架构 —— 每个服务实现 `DeploymentProvider` 协议并在 `ServiceRegistry` 中自动注册。添加新服务只需创建一个新的 Provider 文件。
